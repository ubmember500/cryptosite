...
async function ensureActivitySchema() {
  const provider = String(process.env.DATABASE_URL || '').toLowerCase();
  if (!provider.startsWith('postgres://') && !provider.startsWith('postgresql://')) {
    return;
  }
  ...
  await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "UserActivityEvent_occurredAt_idx" ON "UserActivityEvent"("occurredAt");');
  await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "UserActivityEvent_eventType_occurredAt_idx" ON "UserActivityEvent"("eventType", "occurredAt");');
  await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "UserActivityEvent_userId_occurredAt_idx" ON "UserActivityEvent"("userId", "occurredAt");');
  await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "UserActivityEvent_pagePath_occurredAt_idx" ON "UserActivityEvent"("pagePath", "occurredAt");');
  await prisma.$executeRawUnsafe('CREATE UNIQUE INDEX IF NOT EXISTS "UserDailyActivity_userId_day_key" ON "UserDailyActivity"("userId", "day");');
  await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "UserDailyActivity_day_idx" ON "UserDailyActivity"("day");');
  await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "UserDailyActivity_userId_day_idx" ON "UserDailyActivity"("userId", "day");');
  await prisma.$executeRawUnsafe('CREATE UNIQUE INDEX IF NOT EXISTS "SiteDailyActivity_day_key" ON "SiteDailyActivity"("day");');
  await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "SiteDailyActivity_day_idx" ON "SiteDailyActivity"("day");');
  ...
  await prisma.$executeRawUnsafe(`
    DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'UserDailyActivity_userId_fkey') THEN
