      ...
      <table className="w-full text-sm text-left text-textPrimary">
          ...
          <tr>
            {columns.map((col) => (
              <th
                key={col.key}
                scope="col"
                className="px-6 py-3 font-medium text-textSecondary cursor-pointer hover:text-textPrimary transition-colors"
        ...
        <tbody>
          {data.map((row, i) => (
            ...
            >
              {columns.map((col) => (
                <td key={`${i}-${col.key}`} className="px-6 py-4 whitespace-nowrap">
